...
class Database extends Base
{
    ...
    {
        $engine = array(
            'ARCHIVE', 'BLACKHOLE', 'CSV', 'InnoDB', 'MEMORY', 'MRG_MYISAM', 'MyISAM', 'PERFORMANCE_SCHEMA'
        );
        return $this->util->array_random($engine);
    }
     ...
     * @return string
     */
    public function column(array $addArr = array())
    {
        $default_column = array(
            'id', 'title', 'content', 'name', 'email', 'phone', 'number', 'type', 'status', 'post_date', 'post_modified', 'excerpt', 'ping', 'guid', 'token', 'group', 'category', 'username', 'password', 'comment', 'url', 'avatar', 'update'
        );
        $column = array_merge($default_column, $addArr);
        return $this->util->array_random($column);
    }
    ...
    {
        $type = array(
            'int', 'varchar', 'text', 'date', 'datetime', 'tinyint', 'time', 'timestamp', 'smallint', 'mediumint', 'bigint', 'decimal', 'float', 'double', 'real', 'bit', 'boolean', 'serial', 'blob', 'binary', 'enum', 'set', 'geometry', 'point',
        );
        return $this->util->array_random($type);
    }
}
